...
class DataPlot(pg.GraphicsLayoutWidget):
    ...
    def add_dataset(self, data: Dataset):
        ...
        legend_label = f"{str(data.metadata.get("Wafer #")) or ''} {str(data.metadata.get("Comments")) or ''} {str(data.metadata.get("Constant Supply")) or ''}"
        self.legend.addItem(p, legend_label)
