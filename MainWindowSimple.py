...
from ui_main_window_simple import Ui_MainWindow
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def get_sweep_parameters(self, quick_measurement=False):
        ...
        sweep_multiplier = 10**-3 if self.sm_param_sweep_current.isChecked() else 1
        sweep_compliance_multiplier = 10**-3 if self.sm_param_sweep_voltage.isChecked() else 1
        constant_multiplier = 10**-3 if self.sm_param_constant_current.isChecked() else 1
        ...
        return SweepParameters(
            ...
            self.sm_param_sweep_start.value() * sweep_multiplier,
            (self.sm_param_quick_sweep_step_override.value() if quick_measurement else self.sm_param_sweep_step.value()) * sweep_multiplier,
            self.sm_param_sweep_end.value() * sweep_multiplier,
            ...
            self.sm_param_constant_output.value() * constant_multiplier,
            0.1,
            self.sm_param_constant_output.value() * constant_multiplier,
    ...
    def get_stream_parameters(self):
        ...
        smu_1_multiplier = 10**-3 if self.ds_param_current_1.isChecked() else 1
        smu_1_compliance_multiplier = 10**-3 if self.ds_param_voltage_1.isChecked() else 1
        smu_2_multiplier = 10**-3 if self.ds_param_current_2.isChecked() else 1
        smu_2_compliance_multiplier = 10**-3 if self.ds_param_voltage_2.isChecked() else 1
        ...
        return DatastreamParameters(
            ...
            self.ds_param_output_1.value() * smu_1_multiplier,
            self.ds_param_compliance_1.value() * smu_1_compliance_multiplier,
            Source.VOLTAGE if self.ds_param_voltage_2.isChecked() else Source.CURRENT,
            self.ds_param_output_2.value() * smu_2_multiplier,
            self.ds_param_compliance_2.value() * smu_2_compliance_multiplier,
            measurement_mode,
    ...
    def sm_params_changed(self):
        ...
        if self.sm_param_constant_voltage.isChecked():
            set_input_mode(self.sm_param_constant_output, False, False, Source.VOLTAGE)
            set_input_mode(self.sm_param_constant_compliance, False, True, Source.CURRENT)
        else:
            set_input_mode(self.sm_param_constant_output, False, False, Source.CURRENT)
