...
from MainWindowSimple import MainWindow
...
class Controller(QtCore.QObject):
    ...
    def __init__(self):
        ...
        self.main_window.menubar_sourcemeter_connections.triggered.connect(
            lambda _: self.update_smu_uis(SMUSearchDialog(self.sourcemeters, self.sourcemeter_names, self.main_window).get_sourcemeters(), False)
        )
    ...
    @QtCore.Slot()
    def update_smu_uis(self, new_connections: List[SourceMeter], rename=True):
    ...
    def get_smu_from_label(self, label: str, allow_simulated=True):
        for smu in self.sourcemeters:
            ...
            if f"SMU - {smu.serial_number}" == label or smu.serial_number == label:
                return smu
    ...
    def initialize_sm_dataset(self, constant_supply_value: float):
        ...
        d.constant_supply_value = constant_supply_value
        d.metadata["Comments"] = f"{d.metadata["Comments"]} {constant_supply_value:.3f}"
        ...
        try:
            ...
            with pd.ExcelWriter(self.sm_data_file_path, mode=write_mode, if_sheet_exists=("overlay" if write_mode == "a" else None)) as writer:
                for run in self.sm_last_run:
                    ...
                    run.metadata["Comments"] = f"{run.metadata["Comments"]} {run.constant_supply_value:.3f}"
    ...
    def load_configuration_from_file(self, file_path: str):
        ...
        self.sourcemeter_names = config.get("sourcemeter_names") or self.sourcemeter_names

        self.update_smu_uis(self.sourcemeters)
...
if __name__ == "__main__":
    ...
    controller.main_window.menubar_sourcemeter_connections.setDisabled(True)
    ...
    smu_connection_worker.finished.connect(lambda: controller.main_window.menubar_sourcemeter_connections.setDisabled(False))
